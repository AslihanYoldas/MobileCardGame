...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    Buton onc_secilen_hamle_bilg=null;
    Buton onc_secilen_hamle=null;
    ...
    public void onClick(@NonNull View v){
        ...
        for(int i=0; i< image_Butonlar.length;i++) {
            if (v.getId() == image_Butonlar[i].getId() ) {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for(int l=1; l<9 ;l++){//burada "l" aday kart no'yu temsil ediyor
            ...
            if(var_mi) {
                ...
                oyuncu2.kartlar[oyuncu2.kart_sayisi ] = l;
                oyuncu2.set_kart_sayisi(oyuncu2.kart_sayisi + 1);
                aitlik[l-1]=2;
    ...
    void savas_sonu(double kazanan_oyuncu_no ){
        ...
        Buton ortadaki_kart;
        boolean tercih;
        if(kazanan_oyuncu_no==1){kazanan=oyuncu1;kaybeden=oyuncu2; tercih = true; kazanan_tut="1";}
        else{kazanan=oyuncu2; kaybeden =oyuncu1; tercih=false;kazanan_tut="2";}
    ...
    void oyun_dongusu(){
        ...
        if(secilen_hamle!= null){
            ...
            uretilen_indeks = rastgelelik_bilg.nextInt(8) ;//hangi renk kart oldugu belirlendi
            secilen_kart = oyuncu2.kartlar[uretilen_indeks];

            while(secilen_kart == 0 && oyuncu2.get_kart_sayisi() != 0){
                uretilen_indeks = rastgelelik_bilg.nextInt(8);//hangi renk kart oldugu belirlendi
                secilen_kart = oyuncu2.kartlar[uretilen_indeks];
            }
            Buton temp;
            ...
            //Secilen kart tekrar secilemez(onundeki bir tur boyunca)
            if(secilen_hamle != null && secilen_hamle!= onc_secilen_hamle && secilen_hamle_bilg != onc_secilen_hamle_bilg){
                savas();

                tv_sag.setText(secilen_hamle.getRenk());
                tv_sag_alt.setText(secilen_hamle.tur_adi()+":"+(int) secilen_hamle.getDeger());
                tv_sol.setText(aktif_kart.onceki_kart.getRenk());
                tv_sol_alt.setText(secilen_hamle.tur_adi()+":"+(int) aktif_kart.onceki_kart.getDeger());

                tv_sag.setText(aktif_kart.mevcut_kart.getRenk());
                tv_sag_alt.setText(aktif_kart.mevcut_kart.tur_adi()+":"+(int) aktif_kart.mevcut_kart.getDeger());
                tv_sol.setText(aktif_kart.onceki_kart.getRenk());
                tv_sol_alt.setText(aktif_kart.onceki_kart.tur_adi()+":"+(int) aktif_kart.onceki_kart.getDeger());

                onc_secilen_hamle = secilen_hamle;
                onc_secilen_hamle_bilg = secilen_hamle_bilg;}
            if(kazanan_kart == null){
                ...
                if(oyuncu1.kart_sayisi == 0 || oyuncu2.kart_sayisi == 0){
                    ...
                    i.putExtra("kazanan",kazanan_tut);
                    startActivity(i);
                }
            }
